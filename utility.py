...
# Create classes
class Utilities:  # Class containing reading values
        ...
        try:
            with open(csvfile) as file:
                ...
                for row in reader:
                    ...
                    values.update(v)
                    n += 1
            entries = len(values)  # get the numbers of entries in the csv file
            if values == {0: {}}:
                verbose = 0
...
class Duration:  # Class for the duration
...
def main():
    ...
    utilities = Utilities.read(args.file) # read the the csv file
    ...
    print(output(utilities, args.save)) # print the table of the utilities updated with new readings


def output(utils, file):  # return the table of utilities and save the data in a csv file
    if utils.entries < 2:
        dur = f"\n"
        pass
    else:
...
def settings(args):  # parse the argument given with the filename
    if args.print != None: # print a table from the data in the argument file
        ...
        sys.exit()
    if args.save != None: # save the results to the argument file
    ...
    if args.merge != None: # merge the argument files to a new file and print the new table
        ut = {}
        er = []
        for f in range(2):
        ...
        if er != []:
            ...
            sys.exit()
    if re.search(r"^.+\.csv$", args.file): # open the argument file
        pass
    else:
        print(
...
def date_check(d): # check the format and validity of the given date
    if d == "":
        d = str(date.today())
...
def estimate(utilities, rdate, key):  # calculate an estimate value for the key reading
...
def check_doubles(utilities, reads):  # check if there are other readings for that date
    for l in range(utilities.entries):
        u = utilities.values[l]
        if reads[D] == u[D]:
            print(
...
def period(start, last):  #   calculate days between two dates
    return ordinal(last) - ordinal(start)


def ordinal(iso):  #   convert the date from yyyy-mm-dd format to an ordinal number
    return (date.fromisoformat(iso)).toordinal()


def statistics(utils):  #   calculate some basic statistics on the readings
...
def trend(u, b): # calculate the variance from the previous period
    try:
        s = ""
        u = float(u)
        b = float(b)
...
def de(n):  # return unformated value for a reading
...
def merge(a, b, save):  # merge two sets of readings from two csv files
    if save == None:
        save = "merged.csv"
    for row in b.values:
